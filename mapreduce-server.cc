...
MapReduceServer::MapReduceServer(int argc, char *argv[]) throw (MapReduceServerException) 
  : user(getUser()), host(getHost()), cwd(getCurrentWorkingDirectory()),
    ...
    serverIsRunning(false), workerPool(kNumWorkerThreads) {
...
unsigned short MapReduceServer::computeDefaultPortForUser() {
  size_t hashValue = hash<string>()(user + "@" + host);// enZZsure different than http-proxy default port
...
void MapReduceServer::orchestrateWorkers() throw () {
  ...
  while (true) {
    ...
    workerPool.schedule([this, clientSocket, clientIPAddress]{handleRequest(clientSocket, clientIPAddress);});
  }
}
 ...
 */
void MapReduceServer::handleRequest(int clientSocket, const string& clientIPAddress) throw() { //Make thread safe
  if (verbose) 
...
bool MapReduceServer::surfaceNextFilePattern(std::string& pattern) throw() {
  lock_guard<mutex> lg(filePatternLock);
  cv.wait(filePatternLock, [this]{return (inflight.size() ==0 && unprocessed.size() == 0) || unprocessed.size() > 0;});
...
void MapReduceServer::markFilePatternAsProcessed(const std::string& clientIPAddress, 
  ...
  if((inflight.size() ==0 && unprocessed.size() == 0) || unprocessed.size() > 0){
       lock_guard<mutex> lg(filePatternLock);
       cv.notify_all();
  }
  if (verbose) 
...
void MapReduceServer::spawnMappers() throw() {
  ...
  for(size_t i=0; i<numMappers; i++){
    ...
    threadArr[i] = thread([this, mapperNode, command] { spawnWorker(mapperNode, command); });
  }
  for (size_t i=0; i<numMappers; i++) threadArr[i].join();
...
void MapReduceServer::spawnReducers() throw() {
  size_t numInputs = stageReducerFiles(intermediatePath, unprocessed);
  vector<string> reducerNodes(nodes);
  thread threadArr[numReducers];
  for(size_t i=0; i<numReducers; i++){
    random_shuffle(reducerNodes.begin(), reducerNodes.end());
    const string& reducerNode = reducerNodes[0];
    string command = buildReducerCommand(reducerNode, reducerExecutable, outputPath, numInputs);
    threadArr[i] = thread([this, reducerNode, command] { spawnWorker(reducerNode, command); });
  }
  for (size_t i=0; i<numReducers; i++) threadArr[i].join();
  if(verbose) cout << "Creation of output files now complete." << endl;
  dumpFileHashes(outputPath);
}


size_t MapReduceServer::stageReducerFiles(const std::string& directory, list<string>& files) throw() {
  DIR *dir = opendir(directory.c_str());
  if (dir == NULL) {
    cerr << "Directory named \"" << directory << "\" could not be opened." << endl;
    exit(1); // this is serious enough that we should just end the program
  ...
  while (true) {
    ...
    if(alreadySeen.find(fileName.substr(6, 5)) != alreadySeen.end()) continue; //File ending
    alreadySeen.insert(fileName.substr(6, 5)); //File ending
    string file(directory);
    file += "/";
    file += fileName.substr(6, 5); //File ending + 
    files.push_back(file);
  }

  closedir(dir); // ignore error, since we can still proceed without having closed the directory
  return numInputFiles/(numMappers * numReducers); //Number of total files divided by how many buckets each hashed to.
}
